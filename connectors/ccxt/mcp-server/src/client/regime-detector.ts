 ...
 */
function detectHigherHighs(bars: BarResult[], lookback: number = SWING_LOOKBACK): boolean {
 ...
 */
function detectLowerLows(bars: BarResult[], lookback: number = SWING_LOOKBACK): boolean {
  if (bars.length < lookback * 3) return false;
...
export class RegimeDetector {
   ...
   */
  private computeSnapshot(bars: BarResult[]): IndicatorSnapshot {
   ...
   */
  private detectAtIndex(bars: BarResult[], endIndex: number): { regime: MarketRegime; confidence: number } | null {
   ...
   */
  detect(bars: BarResult[]): { regime: MarketRegime; confidence: number; recommendations: string[] } {
    if (bars.length < 20) {
   ...
   */
  detectTransitions(bars: BarResult[], step: number = 10): { transitions: RegimeTransition[]; currentRegime: MarketRegime } {
   ...
   */
  getHistory(bars: BarResult[], step: number = 10): RegimeHistoryEntry[] {
   ...
   */
  analyze(bars: BarResult[]): RegimeAnalysis {
    if (bars.length < 50) {
